...
import { FaSearchPlus } from 'react-icons/fa'; // Icon for Zoom Hint
...
const ProductDetails = () => {
  ...
  const handleAddToCart = () => {
    addToCart(product, qty, selectedColor || 'Default');
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
        ...
        <div className="lg:col-span-1 hidden lg:flex flex-col gap-4">
            {product.images && product.images.map((img, i) => (
                <img 
                    ...
                    className={`w-full h-24 object-cover cursor-pointer border-2 rounded hover:opacity-80 transition ${mainImage === img ? 'border-primary' : 'border-transparent'}`} 
                />
            ))}
        </div>

        {/* 2. MAIN IMAGE (MAGNIFIER ENABLED) */}
        <div className="lg:col-span-6 relative">
            ...
            >
                ...
                {/* Zoom Hint Icon (Disappears on Hover) */}
                {!isHovered && (
                    <div className="absolute top-4 left-4 bg-white/80 p-2 rounded-full text-gray-600 text-sm flex items-center gap-2 pointer-events-none">
            ...
            <div className="flex lg:hidden gap-3 mt-4 overflow-x-auto pb-2">
                {product.images && product.images.map((img, i) => (
                    <img key={i} src={img} onClick={() => setMainImage(img)} className={`w-20 h-20 object-cover border-2 rounded flex-shrink-0 ${mainImage === img ? 'border-primary' : 'border-gray-200'}`} />
                ))}
            </div>
        </div>
        ...
        <div className="lg:col-span-5 flex flex-col gap-6">
          ...
          {cleanColors.length > 0 && (
            <div>
                <span className="block font-bold text-sm uppercase mb-3">
                    ...
                    {cleanColors.map((color, i) => (
                        <button 
                            ...
                            className={`w-10 h-10 rounded-full border-2 shadow-sm flex items-center justify-center transition-all duration-200 
                                ${selectedColor === color ? 'border-primary ring-2 ring-offset-2 ring-primary scale-110' : 'border-gray-300 hover:scale-110'}`}
          ...
          <div className="flex flex-col gap-4 mt-4">
             ...
             <div className="flex gap-4">
                 <button 
                    ...
                    className="flex-1 bg-white border-2 border-primary text-primary py-4 rounded font-bold uppercase tracking-wide hover:bg-primary hover:text-white transition"
                 >
                    Add to Cart
                 </button>
                 <button 
                    onClick={handleAddToCart} 
                    disabled={product.stock === 0}
                    className="flex-1 bg-green-600 text-white py-4 rounded font-bold uppercase tracking-wide hover:bg-green-700 transition shadow-lg flex items-center justify-center gap-2"
                 >
                    <i className="fa-solid fa-money-bill-wave"></i> Buy with Cash on Delivery
                 </button>
             </div>
          ...
          <div className="mt-6 text-xs text-gray-500 space-y-2">
            <p>✔ Standard delivery across Pakistan: 3-7 working days</p>
            <p>✔ 7-day return policy applied.</p>
          </div>
        </div>
      </div>
    </div>
